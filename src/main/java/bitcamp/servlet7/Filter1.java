 ...
 * => doFilter() : 요청이 들어올 떄마다 호출된다.
 *    - 필터가 수행할 작업에 대한 작업코드를 이 메서드에 둔다.
 * => destroy() : 필터 객체가 소멸될 때 자동으로 호출된다.
 *    - init()에서 준비한 자원을 해제시키는 코드를 이 메서드에 둔다.
 * => 필터에 URL을 연결한다.
 *    - 지정된 URL로 요청이 들어올 때 마다 필터가 호출된다.
 * => 필터 구동 절차
 ...
 *      <!-- 필터 클래스 등록 -->
  ...
  ** 실행 순서 : web.xml은 선언된 순서대로, 애노테이션은 순서x
  ** Filter는 설계기법 중 Chain of Responsibility을 적용하여 설계된 것.
 */
