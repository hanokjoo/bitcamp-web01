/* 주제: 파일 업로드 처리 3
 * => 배치 폴더에 파일을 저장하기
 * => HTML의 img 태그를 사용하여 저장한 파일을 요청하기
 * => 파일명이 중복되지 않게 처리하기
 * 
 */
    ...
    try {
      ...
      for (FileItem item : items) {
        if (item.isFormField()) { // 일반 폼 항목이면,
          ...
          String newfilename = System.currentTimeMillis() + "_" + count++ + item.getName().substring(lastDotIndex);
          
          ServletContext sc = this.getServletContext();
          // ServletContext는 "/upload/파일명"의 실제 배치 경로를 알려준다.
          File file = new File(sc.getRealPath("/upload/" + newfilename));
          item.write(file);
