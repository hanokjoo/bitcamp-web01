    ...
    try {
      ...
      for (FileItem item : items) {
        if (item.isFormField()) { // 일반 폼 항목이면,
          ...
          // => 주의! 프로젝트 폴더에 업로드하는 것이 아니다
          //item.write(new File("C:/Users/BitCamp/git/bitcamp-web01/src/main/webapp/upload/" + item.getName()));
          
          // => 웹 애플리케이션이 배치된 폴더에 두어야 한다.
          ServletContext sc = this.getServletContext();
          // ServletContext는 "/upload/파일명"의 실제 배치 경로를 알려준다.
          out.printf("실제저장될 경로=%s", sc.getRealPath("/upload/" + item.getName()));
          item.write(new File(sc.getRealPath("/upload/" + item.getName())));
        }
      }
