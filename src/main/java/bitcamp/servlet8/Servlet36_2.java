/* 주제: 쿠키 받기
 * => 웹 브라우저는 웹 서버에 요청할 때마다, 웹 서버로부터 받은 쿠키 데이터를 요청 헤더에 첨부하여 보낸다.
 *    물론 웹 서버로부터 받은 쿠키가 없다면 요청 헤더에 첨부할 일도 없을것이다.
 *    그리고 요청할 때 그 서버로부터 받은 쿠키를 첨부하는 것이지, 다른 서버에서 받은 쿠키를 첨부하는 것은 아니다. 
 * => 쿠키의 유효기간
 *    - 웹 브라우저가 쿠키를 보내기 전에 유효기간을 검사하여, 유효기간이 넘은 쿠키는 보관소에서 제거해버린다.
 *      그리고 서버에 보내지 않는다.
 *      즉 일정시간 주기적으로 쿠키를 청소하는 것은 아니다.
 * => HTTP 프로토콜 분석 : 웹 브라우저가 웹 서버로 쿠키 보내기
 *      GET /bitcamp-web01/servlet/servlet36 HTTP/1.1
        Host: localhost:8080
