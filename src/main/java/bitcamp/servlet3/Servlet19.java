 ...
 * => Servlet은 기술은 서버 프로그램을 작은 조각으로 개발할 수 있게 도와주는 기술이다.
 * => Servlet 기술을 만들 때 처음부터 웹 기술과 결합을 고려해서 만든 것은 아니다.
 *    일반적인 목적의 서버 프로그램을 쉽게 만들 수 있게 도와주기 위해 만든 기술이다.
 * => 그러다보니 service()의 파라미터 타입이,
 *    최소한의 요청 정보를 다루는 기능이 있는 ServletRequest와 
 *    최소한의 응답 정보를 다루는 기능이 있는 ServletResponse로 정의하였다.
 * => 그러나 "서블릿 + 웹"dmfh tkdydgksekaus,
 *    HTTP 요청 정보를 다룰 수 있는 HttpServletRequest와
 *    HTTP 응답 정보를 다룰 수 있는 HttpServletResponse 객체이다.
 * => 결론!
 *    HTTP 정보를 다루고 싶다면
 *    ServletRequest를 HttpSevletRequest로 타입 캐스팅하여 사용하라!
 *    ServletResponse를 HttpServletResponse로  타입 캐스팅하여 사용하라!
 */
